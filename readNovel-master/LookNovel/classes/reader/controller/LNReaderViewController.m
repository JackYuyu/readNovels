...
-(void)loadapi:(NSString*)pageurl
{
    // 1.获得请求管理者
        ...
        mgr.responseSerializer=[AFHTTPResponseSerializer serializer];
          ...
          // 2.发送GET请求
          [mgr GET:[NSString stringWithFormat:@"%@%@", @"http://api.smaoxs.com/rauth/book/parser?link=",pageurl] parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
-(void)threeapi1:(NSString*)bid
{
    // 1.获得请求管理者
        ...
        NSString* theurl=[NSString stringWithFormat:@"http://api.smaoxs.com/rauth/book/sites/%@",bid];
          // 2.发送GET请求
          [mgr GET:theurl parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
              NSJSONSerialization *object = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableLeaves error:nil];
              NSDictionary *dict = (NSDictionary *)object;
...
-(void)threeapi2:(NSString*)cid
{
    // 1.获得请求管理者
        ...
        NSString* theurl=[NSString stringWithFormat:@"http://api.smaoxs.com/rauth/book/chapters/%@",cid];
          // 2.发送GET请求
          [mgr GET:theurl parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
              NSJSONSerialization *object = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableLeaves error:nil];
